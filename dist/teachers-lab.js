...
// 프롬프트는 공개 정적 파일이 아니라 Firestore(shared/prompts)에서 로그인 후 로딩된다.
// 새로고침 시 즉시 반영 위해 로컬 캐시를 우선 로드(캐시는 정적 서빙이 아니라 외부 스크래퍼엔 노출 안 됨).
try {
  if (typeof window !== 'undefined' && !window.TYPE_PROMPTS) {
    var _cachedPrompts = localStorage.getItem('cachedServerPrompts');
    if (_cachedPrompts) window.TYPE_PROMPTS = JSON.parse(_cachedPrompts);
  }
} catch (e) {}
...
async function startPracGeneration() {
  ...
  if (!promptsReady(assignment)) return;
...
var HARNESS_TOPIC_SYSTEM = (typeof window !== 'undefined' && window.TYPE_PROMPTS && window.TYPE_PROMPTS.topic && window.TYPE_PROMPTS.topic.harness) || '';  // 폴백 원문 제거(보안) — 서버 로딩값 사용
...
function enterApp(user) {
  ...
  loadServerPrompts();  // 서버(Firestore)에서 .md 프롬프트 로딩
...
// ─── 서버 프롬프트 로딩/업로드 (.md 프롬프트를 정적 파일 대신 Firestore shared/prompts에서) ───
var _MD_TYPES = ['topic', 'blank', 'blank_hard', 'blank_hard_transform'];

// 생성 전 프롬프트 준비 확인 — .md 유형이 배정됐는데 서버 프롬프트가 없으면 막음
function promptsReady(assignment) {
  var needsMd = assignment && assignment.some(function(a){ return _MD_TYPES.indexOf(a.typeId) >= 0; });
  if (!needsMd) return true;
  if (window.TYPE_PROMPTS && window.TYPE_PROMPTS.topic) return true;
  alert('프롬프트를 서버에서 불러오는 중이거나 아직 업로드되지 않았습니다.\n\n관리자(🔐 관리자 탭)에서 "프롬프트 서버에 올리기"로 업로드했는지 확인한 뒤 다시 시도하세요.');
  return false;
}

// 로그인 후 Firestore에서 프롬프트 로딩 → 전역 주입
function loadServerPrompts() {
  if (!fbDb) { setTimeout(loadServerPrompts, 1000); return; }
  fbDb.collection('shared').doc('prompts').get().then(function(doc) {
    if (doc.exists && doc.data()) {
      window.TYPE_PROMPTS = doc.data();
      try { localStorage.setItem('cachedServerPrompts', JSON.stringify(window.TYPE_PROMPTS)); } catch(e){}
      applyTypePrompts();
      try { reloadMemoryForUser(); } catch(e){}
    }
  }).catch(function(e){ console.warn('서버 프롬프트 로드 실패(캐시 사용):', e.message); });
}

// 마스터: 로컬 prompts-seed.json 파일을 Firestore shared/prompts로 업로드
function uploadPromptsToServer(inputEl) {
  if (!isMaster()) { alert('관리자만 사용할 수 있습니다.'); return; }
  ...
  reader.onload = function() {
    var data;
    try { data = JSON.parse(reader.result); } catch(e) { alert('JSON 파싱 실패: ' + e.message); return; }
    if (!data || !data.topic) { alert('올바른 prompts-seed.json 이 아닙니다 (topic 키 없음).'); return; }
    if (!fbDb) { alert('서버 연결이 없습니다.'); return; }
    fbDb.collection('shared').doc('prompts').set(data).then(function() {
      window.TYPE_PROMPTS = data;
      try { localStorage.setItem('cachedServerPrompts', JSON.stringify(data)); } catch(e){}
      applyTypePrompts();
      try { reloadMemoryForUser(); } catch(e){}
      alert('✓ 프롬프트를 서버에 올렸습니다 (' + Object.keys(data).length + '개 유형). 모든 사용자에게 반영됩니다.');
