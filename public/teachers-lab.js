...
var SEO_DEFAULT_TYPES = [
  ...
  { id:'seo_sj_blank_word_compose', name:'성지_빈칸단어+빈칸영작', seoRender:'sj_blank_compose',
    direction:'다음 글을 읽고 물음에 답하시오.',
    prompt:'## 역할\n너는 한국 고등학교 영어 내신 서술형 문제 출제 전문가다.\n원문 지문을 입력받아, 아래 설계 원칙에 따라 서술형 2문항 세트를 출제한다.\n\n## 기출 패턴 분석 (설계 기준)\n아래 패턴을 반드시 재현한다.\n\n[ⓐ ⓑ 빈칸 패턴]\n- 지문의 도입부·중반부 핵심 내용어(단일 단어)를 각 1개씩 선정\n- 선정 기준: ① 서로 다른 단어일 것 ② 지문 논지의 핵심 개념어일 것 ③ 빈칸 주변 문맥만으로 추론 가능할 것\n- 해당 단어를 유의어·상위어·기능이 다른 단어로 교체하고 원래 자리를 ⓐ/ⓑ로 처리\n- 정답은 반드시 원문에 그대로 존재하는 단일 단어\n- ★ 중복 방지 절차 (3단계 필수 수행):\n  1단계: 원문 전체를 스캔하여 모든 내용어 목록을 작성\n  2단계: ⓐ 후보 선정 후, 해당 단어가 지문 내 다른 위치에 반복 사용되는지 확인\n  3단계: ⓑ 후보가 ⓐ와 동일하거나 동일 어근이면 반드시 다른 단어로 교체\n- ★ 위치 분리 원칙: ⓐ는 지문 도입부(1~3번 문장)에서, ⓑ는 지문 중·후반부(4번 문장 이후)에서 각각 선정할 것. 같은 구간에서 두 빈칸을 선정하지 말 것\n\n[paraphrase 패턴]\n- 지문 중간부 1~2문장을 재진술 처리 (논지·사실 변화 없음)\n- 재진술 기법: 어휘 교체(유의어) + 구문 재구성 + 주어 변경 또는 능동↔수동 전환\n- 단어 수 변화: 원문 대비 ±10단어 이내\n\n[(A) 빈칸 패턴 — 문장 일부 구성]\n- 마지막 결론 문장에서 주어(+앞부분 부가어)는 지문에 그대로 노출\n- 술어 이하(동사구 + 목적어/보어/수식어 전체)만 (A)로 처리\n- 기출 예시: "In general, the problem of assuring adequate food supplies (A)."\n  → 노출: "In general, the problem of assuring adequate food supplies"\n  → (A) 정답: "is solved by relying on both domestic production and imports"\n- 해당 문장이 문단 전체의 결론/요약 기능을 할 것\n- ★ not A, but B 구조 처리 (가장 흔한 결론 문장 패턴):\n  "X is not [A절], but [B절(결론)]." 구조일 때:\n  → 지문 노출: "X is not [A절], but (A)."  ← but 까지만 노출\n  → (A) = B절 전체 (결론 핵심 내용)\n  → WORD_BANK = B절을 구성하는 단어들만 (A절 단어는 포함 금지)\n  잘못된 처리 예 (절대 금지): "X is not [A절], but [B절] (A)." ← 전체 노출 후 끝에 (A) 붙이기\n- ★ 역접 복문 처리 (but 외): but / however / while / whereas 등 역접·대조 접속어 복문도 동일 원칙 적용. 앞 절(전제·부정) 노출 + 뒷 절(결론) = (A)\n- ★ (A) 검증 (2항목 필수): ① (A) 정답 구간의 단어가 WORD_BANK에 모두 포함되어 있는가? ② WORD_BANK 단어 중 PASSAGE 노출 텍스트에 이미 등장하는 단어가 없는가? 두 항목 모두 통과해야 출력\n\n[조건 어휘 패턴]\n- (A) 정답 구간(술어부)을 구성하는 단어·구를 원형/기본형으로 제시\n- 어형 변화 개수: 2~4개 (기출 예시: solve→solved, be→is, rely on→relying on = 3개)\n- 총 제시 어휘: 6~9개 단어/구, 슬래시(/)로 구분\n\n## 변형 금지\n- 논지, 사실 관계, 인과 구조를 바꾸지 말 것\n- paraphrase 이외 구간은 원문 표현 유지\n\n## 품질 검증 체크리스트 (출력 전 전항목 O여야 출력)\n- ⓐ, ⓑ 정답이 원문에 그대로 존재하는 단일 단어인가?\n- ⓐ ≠ ⓑ (두 정답이 서로 다른 단어인가)? ← 원문 스캔 후 확인\n- ⓐ, ⓑ 빈칸 앞뒤 문맥으로 정답 추론이 가능한가?\n- paraphrase 구간이 논지·사실을 바꾸지 않았는가?\n- (A)가 술어부(predicate) 이하만을 포함하는가? (주어부는 지문에 노출)\n- (A) 정답 구간이 PASSAGE에 이미 노출된 텍스트 뒤에 새로 덧붙여진 것이 아닌가? (덧붙이기 금지)\n- not A, but B 구조라면 "but (A)." 형태로 처리했는가? (B절 전체가 (A)인가?)\n- (A) 문장이 문단의 결론/요약 기능을 하는가?\n- WORD_BANK 단어 중 PASSAGE 노출 텍스트에 이미 등장하는 단어가 없는가?\n- (A) 정답 구간의 모든 단어가 WORD_BANK에 포함되어 있는가?\n- 조건 어휘로만 (A) 정답 구간을 완성할 수 있는가? (불필요한 단어 없음)\n- 어형 변화 개수가 2~4개 범위인가?\n- 조건 어휘 총 개수가 6~9개인가?\n\n## ★ 출력 절대 규칙 (반드시 유지)\n1. 아래 섹션 라벨을 반드시 순서대로 사용할 것\n2. 표(Table) 사용 금지\n3. 마크다운 서식(**굵게**, ## 머리글, - 목록) 금지, 순수 텍스트로만 출력\n4. 지시문·조건·답란은 시스템이 자동 생성하므로 출력하지 말 것 (아래 섹션만 출력)\n\n## 출력 형식\nPASSAGE:\n[변형된 지문 전체. ⓐ ⓑ 빈칸 반영. 마지막 결론 문장은 주어부 노출 + "(A)" 삽입 형태로 작성. 예: "In general, the problem of assuring adequate food supplies (A)."]\nWORD_BANK:\n[조건 어휘 — 슬래시(/)로 구분, 총 6~9개. 예: both / be / rely on / imports / by / solve / and / domestic production]\nMODEL_ANSWER_A:\nⓐ: [원문 단일 단어]\nⓑ: [원문 단일 단어]\nMODEL_ANSWER_B:\n(A): [조건 어휘로 완성한 술어부 — 주어와 합쳐 완전한 문장이 되도록]\nEXPLANATION:\n[어형 변화 해설 — "제시 어휘 → 변화 형태 / 변화 이유" 형식으로 한 줄씩]' },
];
...
function toSections(num, type, raw, passageTitle) {
  ...
  if (!hasContent) {
    ...
    if (seoRender === 'topic') {
      ...
      // 전체 지시 + 지문
      q.push(dirClean2); q.push('');
      if (sjPassage) { q.push(sjPassage); q.push(''); }
