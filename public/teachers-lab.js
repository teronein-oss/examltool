...
var SEO_DEFAULT_TYPES = [
  ...
  { id:'seo_sj_blank_word_compose', name:'성지_빈칸단어+빈칸영작', seoRender:'sj_blank_compose',
    direction:'다음 글을 읽고 물음에 답하시오.',
    prompt:'## 역할\n너는 한국 고등학교 영어 내신 서술형 문제 출제 전문가다.\n원문 지문을 입력받아, 아래 설계 원칙에 따라 서술형 2문항 세트를 출제한다.\n\n## 입력\n- [원문 지문]: 출제 대상 영어 지문 (전문)\n\n### STEP 1. 지문 변형 설계\n원문을 아래 규칙에 따라 기출 지문으로 변형한다.\n\n변형 규칙:\n① 빈칸 ⓐ, ⓑ 처리 (찾아 쓰기형):\n  - 지문 내 핵심 개념어/구(2단어 명사구 우선)를 2곳 선정\n  - 해당 표현을 유사 상위어(ensuring, achieving, realizing 등)나 의미 범주가 다른 동사로 교체한 뒤, 원래 표현 자리를 빈칸으로 처리\n  - 빈칸 앞뒤 문맥만으로 원문 표현을 추론할 수 있어야 함\n  - 정답은 반드시 원문에서 연속된 2단어로 존재해야 함\n② paraphrase 구간 (1~2문장):\n  - 지문 중간부에서 1~2문장을 선택해 재진술 처리\n  - 재진술 수준: 어휘 교체(유의어) + 구문 재구성, 논지·사실 변화 없음\n  - 단어 수는 원문 대비 ±10단어 이내 유지\n  - 예: 주어 변경, 능동↔수동, 명사구↔절 전환 등\n③ 서술형 빈칸 (A) 처리:\n  - 지문 마지막 문장 전체를 빈칸 (A)로 처리\n  - 원문 마지막 문장은 문단 전체의 결론/요약 기능을 해야 함\n  - 해당 문장이 결론 기능이 약하면, 마지막 두 문장 중 더 적합한 것을 선택\n\n변형 금지 사항:\n  - 논지, 사실 관계, 인과 구조를 바꾸지 말 것\n  - paraphrase 이외 구간은 원문 표현 유지\n\n### STEP 2. 정답 문장 분석 (내부 설계용)\n빈칸 (A)의 원문 정답 문장을 분석한다:\n1. 문장 구조 파악: 주어 / 술어 / 수식 구조 명시\n2. 핵심 표현 목록 추출 (명사구, 전치사구, 접속어 등)\n3. 어형 변화 필요 항목 결정 (동사 시제·수동태·동명사·to부정사 / 명사 단복수). 어형 변화 개수는 2~4개로 제한\n4. 조건 어휘 목록 확정: 원형/기본형으로 제시할 단어와 그대로 제시할 단어 구분. 총 제시 어휘 6~9개 단어/구\n\n## 품질 검증 체크리스트 (출력 전 자가 점검 — 모두 통과해야 출력)\n- ⓐ, ⓑ 정답이 원문에서 연속된 정확히 두 단어인가?\n- ⓐ, ⓑ 빈칸 앞뒤 문맥으로 정답 추론이 가능한가?\n- paraphrase 구간이 논지·사실을 바꾸지 않았는가?\n- (A) 문장이 문단의 결론/요약 기능을 하는가?\n- 조건 어휘로만 (A) 문장을 완성할 수 있는가? (불필요한 단어 없음)\n- 어형 변화 개수가 2~4개 범위인가?\n- 조건 어휘 총 개수가 6~9개인가?\n\n## ★ 출력 절대 규칙 (반드시 유지)\n1. 아래 섹션 라벨을 반드시 순서대로 사용할 것\n2. 표(Table) 사용 금지\n3. 마크다운 서식(**굵게**, ## 머리글, - 목록) 금지, 순수 텍스트로만 출력\n4. 지시문·조건·답란은 시스템이 자동 생성하므로 출력하지 말 것 (아래 섹션만 출력)\n\n## 출력 형식\nPASSAGE:\n[변형된 지문 전체 — ⓐ, ⓑ 빈칸과 (A) 빈칸을 반영한 영어 지문]\nWORD_BANK:\n[조건 어휘 목록 — 슬래시(/)로 구분, 총 6~9개. 예: both / be / rely on / imports / by / solve / and / domestic production]\nMODEL_ANSWER_A:\nⓐ: [원문에서 찾은 두 단어]\nⓑ: [원문에서 찾은 두 단어]\nMODEL_ANSWER_B:\n(A): [조건 어휘로 완성한 정답 문장]\nEXPLANATION:\n[어형 변화 해설 — "제시 어휘 → 변화 형태 / 변화 이유" 형식으로 한 줄씩 나열]' },
];
...
function toSections(num, type, raw, passageTitle) {
  ...
  if (!hasContent) {
    ...
    if (seoRender === 'topic') {
      ...
      // (1) ⓐ ⓑ 찾아쓰기
      q.push('(1) 밑줄 친 ⓐ, ⓑ에 들어갈 문맥상 적절한 말을 본문에서 찾아, 각각 두 단어로 쓰시오.');
