...
var SEO_DEFAULT_TYPES = [
  ...
  { id:'seo_sj_blank_word_compose', name:'성지_빈칸단어+빈칸영작', seoRender:'sj_blank_compose',
    direction:'다음 글을 읽고 물음에 답하시오.',
    prompt:'## 역할\n너는 한국 고등학교 영어 내신 서술형 문제 출제 전문가다.\n원문 지문을 입력받아 아래 순서대로 처리한다.\n\n## ★ 처리 순서 (반드시 이 순서로 수행)\n[PHASE 1: 내부 설계 및 검증] → [PHASE 2: 최종 출력]\nPHASE 1을 완료하고 모든 검증이 통과된 경우에만 PHASE 2를 출력한다.\n\n---\n\n## PHASE 1: 내부 설계 (DESIGN 섹션으로 출력)\n\n### 1-1. ⓐ 후보 선정 (도입부 1~3번 문장에서)\n- 지문 도입부(1~3번 문장)의 핵심 내용어 목록 작성\n- 후보 선정 기준: 지문 논지의 핵심 개념어 / 빈칸 주변 문맥만으로 추론 가능\n- 선정된 ⓐ 후보 단어와 교체할 유의어·상위어 확정\n\n### 1-2. ⓑ 후보 선정 (중·후반부 4번 문장 이후에서)\n- 지문 중·후반부의 핵심 내용어 목록 작성\n- ⓐ와 동일 단어·동일 어근인지 반드시 확인\n- 동일하면 즉시 다른 단어로 교체하여 재선정\n- 선정된 ⓑ 후보 단어와 교체할 유의어·상위어 확정\n\n### 1-3. (A) 구간 설계\n- 마지막 결론 문장 전문 확인\n- 복문 여부 판단: not A but B / however / while / whereas 등\n- 단문이면: SENTENCE_STEM = 주어부, (A) 정답 구간 = 술어 이하\n- not A but B 구조이면: SENTENCE_STEM = "X is not [A절], but", (A) 정답 구간 = B절 전체\n- ★ (A) 길이 초과 처리: (A) 정답 구간을 WORD_BANK로 만들면 9개를 초과하는 경우,\n  우선순위 1 — so/so that/in order to 등 목적·결과 부사절이 있으면 그 절만 (A)\n             SENTENCE_STEM = 마지막 문장에서 해당 부사절 바로 앞까지\n  우선순위 2 — 문장 후반부 핵심 구만 (A)\n  우선순위 3 — 위 방법으로도 9개 초과이면 앞 문장 중 결론 기능 문장으로 재선택\n- ★ SENTENCE_STEM 작성 원칙: 마침표 없이 작성. (A) 뒤 마침표는 렌더 시스템이 자동 추가함\n- (A) 정답 구간 확정 후 WORD_BANK 후보 단어 목록 작성 (원형/기본형)\n\n### 1-4. WORD_BANK 생성 — 단어 분해 파이프라인 (반드시 이 순서로 수행)\n\n[STEP A] (A) 정답 구간 전문을 공백 기준으로 낱말 단위로 분해하여 번호를 붙여 나열한다.\n  예: "is solved by relying on both domestic production and imports"\n  → 1:is 2:solved 3:by 4:relying 5:on 6:both 7:domestic 8:production 9:and 10:imports\n\n[STEP B] 각 번호의 낱말을 원형/기본형으로 변환한다.\n  → 1:be 2:solve 3:by 4:rely 5:on 6:both 7:domestic 8:production 9:and 10:import\n  ★ 절대 생략 금지 항목: be(is/am/are/was/were) / and / or / so / that / the / your / their / them / can / will / have\n  이 단어들이 정답 구간에 있으면 무조건 원형으로 WORD_BANK에 포함\n\n[STEP C] 인접한 단어를 구(phrase)로 묶을 수 있는지 판단한다. (예: rely on → 묶기 가능, domestic production → 묶기 가능)\n  단, 구로 묶으면 총 어휘 수가 줄어드는 경우에만 묶는다. 어휘 수가 6~9개가 되도록 조정.\n\n[STEP D] STEP B/C의 결과가 곧 WORD_BANK이다. 추가하거나 빼지 말 것.\n\n[STEP E] 검증\n- 검증 A: STEP A의 낱말 전체가 STEP B/C 결과(WORD_BANK)로 복원 가능한가? (누락 낱말 없음)\n- 검증 B: WORD_BANK 단어 중 PASSAGE 노출 텍스트에 이미 등장하는 단어가 없는가?\n- 어형 변화 개수 확인: 2~4개 범위인가?\n- 총 어휘 수 확인: 6~9개인가?\n- 검증 A 또는 B 실패 시: 1-3으로 돌아가 (A) 구간 재설계\n\n### 1-5. 최종 검증 체크리스트\n- ⓐ ≠ ⓑ (서로 다른 단어)?\n- ⓐ, ⓑ 각각 원문에 그대로 존재하는 단일 단어인가?\n- WORD_BANK 검증 A, B 모두 통과?\n- (A)가 결론/요약 기능을 하는가?\n→ 전항목 O이면 PHASE 2로 진행. X가 있으면 해당 단계로 돌아가 재설계.\n\n---\n\n## PHASE 2: 최종 출력\n\n## ★ 출력 절대 규칙\n1. 아래 섹션 라벨을 반드시 순서대로 사용할 것\n2. 표(Table) 사용 금지\n3. 마크다운 서식(**굵게**, ## 머리글, - 목록) 금지, 순수 텍스트로만 출력\n4. 지시문·조건·답란은 시스템이 자동 생성하므로 출력하지 말 것\n\n## 출력 형식\nDESIGN:\nⓐ 후보: [단어] / 위치: [문장 번호] / 교체어: [단어]\nⓑ 후보: [단어] / 위치: [문장 번호] / 교체어: [단어]\n중복 확인: ⓐ=[단어] ⓑ=[단어] → 서로 다른 단어 [O/X]\n(A) 마지막 문장: [전문]\n(A) 구조: [단문/not A but B/so부사절/기타]\nSENTENCE_STEM: [...] ← (A) 앞부분만, 마침표 없이\n(A) 정답 구간: [...]\nSTEP A 낱말 분해: [1:단어 2:단어 3:단어 ...]\nSTEP B 원형 변환: [1:원형 2:원형 3:원형 ...]\nSTEP C 구 묶기 결과: [단어1 / 단어2 / ...]\n검증A(낱말 전체 복원 가능): [O/X]\n검증B(SENTENCE_STEM·PASSAGE 텍스트와 겹침 없음): [O/X]\n어형 변화 수: [숫자]\n총 어휘 수: [숫자]\n최종 판정: [통과 / 재설계]\nPASSAGE:\n[변형된 지문에서 마지막 결론 문장을 제외한 나머지 전체. ⓐ ⓑ 교체어로 빈칸 처리. 마침표로 끝낼 것]\nSENTENCE_STEM:\n[DESIGN에서 확정된 SENTENCE_STEM — 마침표 없이. 렌더 시스템이 이 뒤에 " (A) ."를 자동 추가함]\nWORD_BANK:\n[DESIGN STEP C 결과 — 슬래시(/)로 구분]\nMODEL_ANSWER_A:\nⓐ: [DESIGN에서 확정된 ⓐ 후보 단어]\nⓑ: [DESIGN에서 확정된 ⓑ 후보 단어]\nMODEL_ANSWER_B:\n(A): [DESIGN에서 확정된 (A) 정답 구간]\nEXPLANATION:\n[실제로 어형이 변화된 항목만 — "제시 어휘 → 변화 형태 / 변화 이유" 형식으로 한 줄씩. 변화 없는 항목은 포함하지 말 것]' },
];
