...
var SEO_DEFAULT_TYPES = [
  ...
  { id:'seo_summary_4_bh', name:'교과서 요약문 빈칸4개_BH', seoRender:'summary4',
    direction:'[서술형] 윗글의 내용을 다음과 같이 요약하고자 한다. 빈칸 (A)~(D)에 들어갈 알맞은 말을 본문에서 찾아 각각 1~2단어로 쓰시오. [8.0점/부분점수 있음]',
    prompt:'당신은 한국 고등학교 영어 내신/수능형 서술형 문항 출제 전문가입니다.\n주어진 영어 지문을 바탕으로, 본문 내용을 한 문단으로 요약한 뒤 핵심어 4개를 빈칸 처리한 [요약문 빈칸 완성형] 서술형 문항을 제작합니다.\n\n## 1. 지문 가공 원칙 (원문 → 내신형 변형)\n- 구어체 표현 제거 ("Now, here\'s the twist", "Impressive, right?" 등 삭제)\n- 1·2인칭 호명·수사적 질문 제거 → 3인칭 객관 서술, 격식체 문어로 통일\n- 일상 어휘를 수능 빈출 고급 어휘로 치환 (문맥 적합성·고2~고3 수준 유지)\n- 모호한 부분 구체화, 핵심 개념 부연 추가 (1~2문장)\n- 가공 후 지문: 220~260단어, 문단 2~3개\n\n## 2. 요약문 작성 원칙\n- 2~3문장, 총 60~80단어\n- 본문을 그대로 옮기지 않고 동의어 치환·품사 변환·명사구 압축·상위 개념화 기법으로 재서술\n- 본문 논리 흐름 순서 유지 (도입 → 전개 → 결론)\n\n## 3. 빈칸 설계 원칙\n- 정확히 4개 (A)(B)(C)(D), 각 2점, 총 8점\n- 각 답: 1단어 또는 2단어 (3단어 이상 금지)\n- 난이도 점진 상승 구조:\n  (A) 직접 일치형 ★☆☆ — 본문 어휘 그대로 인지\n  (B) Paraphrase 매칭형 ★★☆ — 동의어 관계 파악 후 본문 어휘 추출\n  (C) 핵심어 추출형 ★★☆ — 주제 관통 추상명사 추출\n  (D) 추론·일반화형 ★★★ — 구체 예시를 상위 개념어로 추상화\n- 4개 답의 품사 분산 (모두 명사 금지)\n\n## 4. 정답 해설\n각 빈칸: 정답(1~2단어) / 출처 문장 / 난이도(★☆☆~★★★) / 출제 의도\n\n## ★ 출력 절대 규칙\n1. 아래 섹션 라벨을 반드시 순서대로 사용할 것\n2. 표(Table) 사용 금지\n\n## 출력 형식\nPASSAGE:\n[가공된 본문 — 220~260단어, 고급 어휘·구문으로 변형]\nDIRECTION:\n윗글의 내용을 다음과 같이 요약하고자 한다. 빈칸 (A)~(D)에 들어갈 알맞은 말을 본문에서 찾아 각각 1~2단어로 쓰시오. [8.0점/부분점수 있음]\nSUMMARY:\n[(A)(B)(C)(D) 빈칸이 포함된 요약문 — 60~80단어]\nMODEL_ANSWER:\n(A): [정답] [2.0점]\n(B): [정답] [2.0점]\n(C): [정답] [2.0점]\n(D): [정답] [2.0점]\nEXPLANATION:\n(A) 정답: [답] / 출처: [본문 문장] / 난이도: ★☆☆ / 출제 의도: [설명]\n(B) 정답: [답] / 출처: [본문 문장] / 난이도: ★★☆ / 출제 의도: [설명]\n(C) 정답: [답] / 출처: [본문 문장] / 난이도: ★★☆ / 출제 의도: [설명]\n(D) 정답: [답] / 출처: [본문 문장] / 난이도: ★★★ / 출제 의도: [설명]' },
];
...
function toSections(num, type, raw, passageTitle) {
  ...
  if (!hasContent) {
    ...
    if (seoRender === 'topic') {
      ...
      if (seoCond2) { q.push('< 조건 >'); q.push(seoCond2); q.push(''); }
      q.push('(A) : ____________________  [2.0점]');
      q.push('(B) : ____________________  [2.0점]');
      q.push('(C) : ____________________  [2.0점]');
      q.push('(D) : ____________________  [2.0점]'); q.push('');
